...
> [!NOTE]
> このメジャーリリースより、Laravel 11 と PHP 8.2 はサポート対象外となりました。最低 PHP バージョンを 8.3 へ引き上げ、Laravel のサポートを 12 と 13 に絞っています。Laravel 11 または PHP 8.2 をお使いの場合は、本バージョンをインストールする前にアップグレードするか、従来のメジャーラインをご利用ください。
