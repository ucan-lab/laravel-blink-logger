...
> [!NOTE]
> Laravel 10 はサポート対象外です。Laravel 10.x のすべてのリリースはセキュリティアドバイザリの対象となっており、インストールできなくなったためサポートを終了しました。Laravel 10 をお使いの場合は、本パッケージをインストールする前に Laravel 11 以降へアップグレードしてください。
